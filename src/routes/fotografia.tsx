...
function PhotographyPage(){
  ...
  return <main><PageIntro number="04" title="Photography">Historias capturadas en un instante. Una selección real de eventos, retratos y momentos cotidianos.</PageIntro><div className="flex gap-2 overflow-x-auto border-b border-border px-5 py-4 md:px-10">{photoCategories.map(item=><button key={item} onClick={()=>{setCategory(item);setActive(null)}} className={`shrink-0 border px-4 py-2 font-mono text-[10px] uppercase ${category===item?'border-primary bg-primary text-primary-foreground':'border-border'}`}>{item}</button>)}</div><section className="columns-1 gap-4 p-4 sm:columns-2 md:p-10 lg:columns-3">{visible.map((photo,index)=><button key={photo.src} onClick={()=>setActive(index)} className="group mb-4 block w-full break-inside-avoid overflow-hidden bg-muted text-left"><img src={photo.src} alt={photo.alt} loading="lazy" className="h-auto w-full transition duration-500 group-hover:scale-[1.02]"/><span className="block border-b border-border py-3 font-mono text-[10px] uppercase">{photo.category} / Jhona Fotografía</span></button>)}</section>{activePhoto&&<MediaLightbox images={visible.map(photo=>photo.src)} title={activePhoto.category} startIndex={active??0} onClose={()=>setActive(null)}/>}</main>;
}